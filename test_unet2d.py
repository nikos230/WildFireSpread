...
def test(dataset_path, checkpoints, num_filters, kernel_size, pool_size, use_batchnorm, final_activation, drop_out_rate, train_years, validation_years, threshold, num_layers, checkpoint_path, test_countries):
    ...
    for i in range(0, n_rows):
        ...
        predicted_shapefile = f'WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/predicted_shapefiles/{file_name}_predicted.shp'
        os.makedirs('WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/predicted_shapefiles', exist_ok=True)  # Create directory if not exists
        ...
        ground_truth_shapefile = f'WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/groud_truth_shapefiles/{file_name}_ground_truth.shp'
        os.makedirs('WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/groud_truth_shapefiles', exist_ok=True)  # Create directory if not exists
    ...
    output_path = 'WildFireSpread/WildFireSpread_UNet2D/output_plots/test_results_with_overlap.png'
    os.makedirs('WildFireSpread/WildFireSpread_UNet2D/output_plots')
    plt.savefig(output_path)
    plt.close()  # Close the figure to free up memory

    print(f'Saved visualization of test samples (with overlap) to {output_path}')

    combine_shp('WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/groud_truth_shapefiles', 'WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/ground_truth_combined/ground_truth_combined.shp')
    combine_shp('WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/predicted_shapefiles', 'WildFireSpread/WildFireSpread_UNet2D/out_shapefiles/predicted_combined/predicted_combined.shp')
