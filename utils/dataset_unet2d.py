...
class BurnedAreaDataset(Dataset):
    ...
    def load_data(self):
        for file in self.nc_files:
            ...
            #sample = sample.isel(time=slice(4, 5)) 
            ...
            # get dynamic variables from sample .nc file
             # Load dynamic variables and combine all time steps into the channel dimension
            #for variable in dynamic_vars:
                #data_array = sample[variable].values  # Shape: (time_steps, H, W)
                # Reshape to (time_steps, H, W) and append each time step as a separate channel
                #for t in range(time_steps):
                    #inputs.append(data_array[t])  # Append each time step as a separate channel

            for t in range(time_steps):
                for variable in dynamic_vars:
                    #if variable == 'ndvi' and t != 4:
                        #continue
                    data_array = sample[variable].values  # Shape: (time_steps, H, W)
                    inputs.append(data_array[t])  # Append the variable at the current time step    
                    #print(f'Time Step: {t}, Variable: {variable}')
            #exit()            
                    

           # Load static variables and append directly (these do not vary over time)
            for variable in static_vars:
                data_array = sample[variable].values  # Shape: (H, W)
                inputs.append(data_array)  # Append the static variable directly as a channel
            ...
            # label varible
            label = sample['burned_areas'].values[4] # get label for time=2 (first fire day) 
...
if __name__ == '__main__':
    samples_path = ['WildFireSpread/WildFireSpread_UNet3D/dataset/dataset_sampled_corrected/2015/corrected_sample_0.nc']
