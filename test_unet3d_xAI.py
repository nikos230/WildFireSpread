...
def test(dataset_path, checkpoints, num_filters, kernel_size, pool_size, use_batchnorm, final_activation, drop_out_rate, train_years, validation_years, threshold, num_layers, checkpoint_path, test_countries):
    ...
    for idx, (inputs, targets) in enumerate(test_loader):
        ...
        save_saliency_map(idx, saliency, dynamic_vars, static_vars, targets, inputs)
...
def save_saliency_map(idx, saliency_maps, dynamic_vars, static_vars, targets, inputs):
    # saliency_maps shape [batch_size, channel, time_step, height, width]
    all_variables_names = dynamic_vars + static_vars
    path_to_maps = f'xAI_UNet3D/sample_{idx}'
